...
const growthPattern = new IntegerProperty({
  init: 0,
  enumVals: ['dichotomous', 'monopodial', 'dichotomous2'],
});
...
// const dichotomous2 = new ComputedProperty(
//   growthPattern,
//   value => value === GrowthPattern.Dichotomous2
// );
...
export class MeshGenerator {
  ...
  private setFilePath(filePath: string) {
    const fname = window.bridge.path.basename(filePath);
    let name = fname;
    const index = name.lastIndexOf('.');
    this.name = name.substr(0, index);
  ...
  private growBranch(positionArray: number[], indexArray: number[]) {
    ...
    const monopodial = growthPattern.value === GrowthPattern.Monopodial;
    const dichotomous2 = growthPattern.value === GrowthPattern.Dichotomous2;
    ...
    const grow = (
      ...
      if (monopodial) {
          ...
          while (currentLength < branchLength && maxIterations > 0) {
            ...
            while (nextFork <= currentLength && nextFork < branchLength) {
              ...
              for (let i = 0; i < symmetry; i += 1) {
                const yAngle = branchRotation + (i * Math.PI * 2) / symmetry;
                const xAngle = -branchAngle;
                const offset = nextFork - currentLength;
                const length2 = this.rnd.next(...length);
                const radius = forkRadius * 0.9;
                const scale = branchScale * (1 - relativePosition * lengthTaper);
                ...
                grow(forkTransform, length2, radius, scale, branchLevel + 1);
              }
        ...
        if (branchLevel < branchStack.length) {
          ...
          const { angle, axis, length, symmetry } = branchStack[branchLevel].values;
      ...
      // Endpoint
      transform.multiply(tmpMatrix.makeTranslation(0, branchRadius, 0));
